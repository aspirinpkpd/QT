---
title: \vspace{8cm}\LARGE\"Population PK-QT Analysis Example"\vspace{4cm} 
  ![](logo.jpg){width=5in}  
...
Study subjects were wearing a Holter monitor for electrocardiogram (ECG) recording for 24 hrs. Ten ECG recordings and blood samples for PK analysis were collected at each time point. All study treatments had identical sample
...
- Examining the mean concentration and placebo-corrected change from baseline QTcF ($\Delta \Delta$QTcF) profiles by dose level. 
...
```{r Figure Change from baseline per treatment, warning=FALSE, include=TRUE}
...
```{r, Data Summaries deltaQTcF, include=TRUE}
...
```{r Figures of Moxifloxacin PK Profiles, warning=FALSE, include=TRUE}
...
```{r, Summary of Moxifloxacin PK concentration over time, include=TRUE}
...
```{r HR effect summary table, warning=FALSE, include=TRUE}
...
```{r time delay effect check HR, warning=FALSE, include=TRUE}
...
```{r HR plot, warning=FALSE, include=TRUE}
...
```{r QT correction, warning=FALSE, include=TRUE}
...
```{r time delay effect check DQT, warning=FALSE, include=TRUE}
# Mean Baseline corrected placebo effect
...
```{r time delay loess DQT, warning=FALSE, include=TRUE}
...
```{r QTc-Concentration Analysis, warning=FALSE, include=TRUE}
#
...
```{r QTc-Concentration Analysis Table, warning=FALSE, include=TRUE}
...
unity and loess line are presented in Figure \ref{fig:pred_ipred} and do not suggest any patterns that indicate over or under model-predicted $\Delta$QTc.

```{r Prediction-Based Diagnosis, warning=FALSE, include=TRUE}
...
```{r Residual-Based diagnostics, warning=FALSE, include=TRUE}
...
```{r Boxplots and q-q plots, warning=FALSE, include=TRUE}
...
```{r boxplots time,warning=FALSE, include=TRUE}
...
```{r boxplots treatment,warning=FALSE, include=TRUE}
...
```{r Quantiles DeltaQTcF, warning=FALSE, include=TRUE}
...
# Discussion
The ICH [E14 Q&A (R3)](https://database.ich.org/sites/default/files/E14_Q%26As_R3_Q%26As.pdf) document was revised in December 2015 to confirm concentration-QTc (C-QT) analyses as an acceptable primary analysis methodology for definitive QTc characterization. Subsequently, a [scientific white paper on concentration-QTc modeling](https://link-springer-com.eu1.proxy.openathens.net/content/pdf/10.1007/s10928-017-9558-5.pdf) was published collaboratively by regulatory and industry representatives. This paper described the critical design elements for the studies suitable for such analysis, as well as modeling and reporting aspects corresponding to regulatory submissions involving such concentration-QTc analyses. The definitive C-QT analysis of data in early phase studies following ICH E14 Q&A (R3) implementation has alleviated the need for Thorough QT/QTc (TQT) studies for multiple development programs. In this example report we used moxifloxacin data from the [IQ-CSRC study](https://pubmed.ncbi.nlm.nih.gov/25670536/) .
...
This analysis represents the quantitative evaluation of the relationship between changes in the placebo-adjusted change from baseline QTc interval and plasma concentrations of moxifloxacin. 
 ...
 - The final QTc_Concentration model for $\Delta$QTcF indicated a mean estimate of a `r table[[20,2]]`  msec increase
from baseline in QTcF per ng/mL of moxifloxacin. At the geometric mean therapeutic Cmax  value observed in patients (400 mg dose), the mean predicted increase in placebo-adjusted change from baseline QTcF is `r round(thera.pred.moxi$Contrast[[1]],2)` msec, and the upper bound of the 90% CI of $\Delta \Delta$QTcF is above 10 msec (`r round(thera.pred.moxi$Upper[[1]],2)`).

- At the geometric  mean supratherapeutic moxifloxacin Cmax value observed in patients (800 mg dose), the mean predicted increase in placebo-adjusted change from baseline QTcF is `r round(supra.pred.moxi$Contrast[[1]],2)` msec and the upper bound of the 90% CI of $\Delta \Delta$QTcF  is above the 10 msec threshold (`r round(supra.pred.moxi$Upper[[1]],2)`).
